-- Corriger la fonction handle_new_user pour utiliser la bonne extension et syntaxe HTTP
  ...
  IF NOT is_team_member THEN
    ...
    IF company_address IS NOT NULL AND company_address != '' THEN
      ...
      BEGIN
        ...
        RAISE LOG 'HTTP Status for geocoding: %, Content: %', http_response.status, http_response.content;
        ...
        IF http_response.status = 200 THEN
          ...
          EXCEPTION
            WHEN OTHERS THEN
              RAISE LOG 'Error parsing JSON response: %, Content: %', SQLERRM, http_response.content;
          END;
        ELSE
          RAISE LOG 'HTTP error during geocoding: Status %, Content: %', http_response.status, http_response.content;
        END IF;
      EXCEPTION
        WHEN OTHERS THEN
