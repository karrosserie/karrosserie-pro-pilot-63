-- Modifier la fonction trigger pour appeler directement l'edge function process-documents-request
    ...
    IF NEW.client_id IS NOT NULL AND NEW.vehicle_id IS NOT NULL THEN
        ...
        IF NEW.company_id IS NOT NULL THEN
            ...
            IF (client_email IS NOT NULL AND client_email != '') OR (client_phone IS NOT NULL AND client_phone != '') THEN
                ...
                -- Appeler l'edge function process-documents-request de manière asynchrone
                PERFORM http_post(
                ...
                RAISE LOG 'Auto-created quote and token for expertise report: %, token: %, calling process-documents-request', NEW.id, token_id;
            ELSE
