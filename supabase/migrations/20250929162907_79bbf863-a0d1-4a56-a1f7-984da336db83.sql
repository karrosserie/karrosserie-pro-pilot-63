...
-- Update the test function to use the correct approach
...
BEGIN
  ...
  -- Use standard http_post function
  WITH http_result AS (
    SELECT * FROM http_post(
      'https://jukdsypvuehnniskgpfd.supabase.co/functions/v1/geocode-company-address',
      json_build_object('address', address_param)::text,
      'application/json'
    )
  )
  SELECT status, content INTO http_status, http_content
  ...
  RAISE LOG 'HTTP Response Status: %, Content: %', http_status, http_content;
  ...
  ELSE
    RAISE LOG 'HTTP Error: Status %, Content: %', http_status, http_content;
    RETURN jsonb_build_object('error', 'HTTP Error', 'status', http_status, 'content', http_content);
  END IF;
