-- Améliorer la fonction handle_new_user avec un meilleur logging et gestion d'erreur pour l'appel HTTP
  ...
  IF NOT is_team_member THEN
    ...
    IF company_address IS NOT NULL AND company_address != '' THEN
      ...
      BEGIN
        ...
        RAISE LOG 'HTTP Status du géocodage: %, Headers: %', 
        ...
        IF http_response.status = 200 THEN
          ...
          END;
        ELSE
          RAISE LOG 'Erreur HTTP lors du géocodage: Status %, Content: %', 
                    http_response.status, 
