-- Mise à jour de la fonction get_available_employees avec le système de priorisation par rôle
-- Priorisation : Carrossier > Carrossier-Véhicule de courtoisie > Responsable > Propriétaire
...
BEGIN
  ...
  SELECT 
    ...
    -- Priorisation : Carrossier (100) > Carrossier-Véhicule de courtoisie (90) > Responsable (50) > Propriétaire (30)
    CASE 
      WHEN uc.role = 'Carrossier' THEN 100
      WHEN uc.role = 'carrossier' THEN 100  -- Support ancien format
      WHEN uc.role = 'Carrossier-Véhicule de courtoisie' THEN 90
      WHEN uc.role = 'carrossier-vehicule de courtoisie' THEN 90  -- Support ancien format
      WHEN uc.role = 'Responsable' THEN 50
      WHEN uc.role = 'Manager' THEN 50  -- Support ancien format
      WHEN uc.role = 'Propriétaire' THEN 30
      ELSE 20  -- Autres rôles (pas prioritaires)
  ...
  ORDER BY 
    -- D'abord par priorité de rôle (carrossiers en premier, puis responsables, puis propriétaires)
    role_priority DESC,
