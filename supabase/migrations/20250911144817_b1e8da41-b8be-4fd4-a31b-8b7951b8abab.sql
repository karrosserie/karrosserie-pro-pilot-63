-- Modifier la fonction auto_create_quote_from_expertise_report pour enlever l'appel HTTP direct
-- et utiliser pg_notify() pour déclencher l'envoi asynchrone
    ...
    IF NEW.client_id IS NOT NULL AND NEW.vehicle_id IS NOT NULL THEN
        ...
        IF NEW.company_id IS NOT NULL THEN
            ...
            IF (client_email IS NOT NULL AND client_email != '') OR (client_phone IS NOT NULL AND client_phone != '') THEN
                ...
                -- Utiliser pg_notify pour déclencher l'envoi asynchrone
                -- Cela permettra à l'edge function d'être appelée après le commit de la transaction
                PERFORM pg_notify('send_documents_request', jsonb_build_object(
                    'tokenId', token_id::text,
                    'clientEmail', client_email,
                    'clientPhone', client_phone
                )::text);
                
                RAISE LOG 'Auto-created quote and token for expertise report: %, token: %', NEW.id, token_id;
            ELSE
