...
-- Améliorer la fonction handle_new_user avec plus de logging et gestion d'erreur
...
BEGIN
  ...
  IF NOT is_team_member THEN
    ...
    IF NEW.raw_user_meta_data->>'company_address' IS NOT NULL AND 
      ...
      RAISE LOG 'Tentative de géocodage pour adresse: %', NEW.raw_user_meta_data->>'company_address';
      ...
      BEGIN
        ...
        RAISE LOG 'Réponse du géocodage reçue: %', geocode_response;
        ...
        IF geocode_response IS NOT NULL AND geocode_response->>'success' = 'true' THEN
          ...
          RAISE LOG 'Géolocalisation réussie pour adresse: %, lat: %, lon: %', 
                    ...
                    company_longitude;
        ELSE
          RAISE LOG 'Échec de la géolocalisation pour adresse: %, réponse: %', 
                    NEW.raw_user_meta_data->>'company_address',
                    geocode_response;
        END IF;
      EXCEPTION
        WHEN OTHERS THEN
          GET STACKED DIAGNOSTICS geocode_error = MESSAGE_TEXT;
          RAISE LOG 'Erreur lors de la géolocalisation: %, détails: %', SQLERRM, geocode_error;
