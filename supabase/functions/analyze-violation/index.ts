...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Analyzing violation:', { violationId, documentUrl, companyId });

    if (!violationId || !documentUrl || !companyId) {
      throw new Error('Missing required parameters: violationId, documentUrl, or companyId');
    }
    ...
    // Update the violation with the extracted information
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Violation analyzed and updated successfully',
    ...
    console.error('Error in analyze-violation function:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
