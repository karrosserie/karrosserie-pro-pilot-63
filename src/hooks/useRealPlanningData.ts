...
export const useRealPlanningData = (companyId: string | null) => {
  ...
  const fetchPlanningData = useCallback(async () => {
    try {
      ...
      // Récupérer les tâches programmées avec jointures (exclure les tâches "En attente")
      console.log('📡 Querying employee_schedule with joins (excluding "En attente" tasks)...');
      const { data, error } = await supabase
        ...
        .neq('status', 'En attente') // Exclure les tâches en attente du planning normal
  ...
  // Fonction pour récupérer spécifiquement les tâches en attente
  const fetchWaitingTasks = useCallback(async () => {
    try {
      console.log('🚀 fetchWaitingTasks START - companyId:', companyId);
      
      if (!companyId) {
        console.log('❌ fetchWaitingTasks: Cannot fetch data - companyId is null/undefined');
        return [];
      }

      // Récupérer uniquement les tâches avec statut "En attente"
      console.log('📡 Querying waiting tasks...');
      const { data, error } = await supabase
        .from('employee_schedule')
        .select(`
          *,
        ...
        `)
        .eq('company_id', companyId)
        .eq('status', 'En attente') // Récupérer uniquement les tâches en attente
        .order('start_datetime', { ascending: true });

      console.log('📦 Waiting tasks query result:', {
        dataLength: data?.length || 0,
        error: error,
        companyId,
        rawData: data
      ...
      if (!data || data.length === 0) {
        console.log('⚠️ No waiting tasks found for companyId:', companyId);
        return [];
      }

      // Convertir les tâches en attente au même format que les tâches normales
      const waitingTasks = data.map((item: any) => {
        const startDate = new Date(item.start_datetime);
        const endDate = new Date(item.end_datetime);
        ...
        return {
          ...
          jour: 'en_attente', // Jour spécial pour les tâches en attente
          user_id: item.user_id,
          vehicle_id: item.vehicle_id,
          task_type: item.task_type,
          start_datetime: item.start_datetime,
          end_datetime: item.end_datetime,
          real_start_datetime: item.real_start_datetime,
          real_end_datetime: item.real_end_datetime,
          reason: 'Pièces manquantes' // Raison par défaut, pourrait être dynamique
        };
      });

      console.log('✅ Waiting tasks loaded:', waitingTasks);
      return waitingTasks;

    } catch (error) {
      console.error('❌ Unexpected error fetching waiting tasks:', error);
      return [];
    }
  }, [companyId]);
