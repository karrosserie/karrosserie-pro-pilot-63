...
const ClientCreditsTab: React.FC<ClientCreditsTabProps> = ({ clientId }) => {
  ...
  const formatVehicleDisplay = (credit: any) => {
    if (credit.vehicles) {
      let brand = credit.vehicles.car_brands?.name || '';
      let model = credit.vehicles.car_models?.name || '';
    ...
    if (credit.invoice_id && invoices) {
      ...
      if (linkedInvoice?.vehicles) {
        let brand = linkedInvoice.vehicles.car_brands?.name || '';
        let model = linkedInvoice.vehicles.car_models?.name || '';
  ...
  const getInvoiceDisplay = (invoiceId: string | null) => {
    if (!invoiceId || !invoices) return 'Sans facture';
  ...
  const handleView = (credit: any) => {
    if (credit.invoice_id && invoices) {
      ...
      if (relatedInvoice) {
        ...
        toast({ title: "Facture introuvable", description: "La facture associée à cet avoir n'a pas été trouvée.", variant: "destructive" });
      }
    } else {
      toast({ title: "Aucune facture associée", description: "Cet avoir n'est pas lié à une facture.", variant: "destructive" });
    }
  };
  ...
  const handleEdit = (credit: any) => {
    ...
    if (credit.items_data) {
      try { items = JSON.parse(credit.items_data); } catch (error) { console.error('Error parsing items_data:', error); }
    }
    setSelectedCredit({ ...credit, items });
  ...
  const handleDelete = async (credit: any) => {
    ...
    if (confirmed) {
      try { await deleteCredit.mutateAsync(credit.id); } catch (error) { console.error('Error deleting credit:', error); }
    }
  };

  const handleDownload = async (credit: any) => {
    try {
      toast({ title: "Génération du PDF", description: "Génération du PDF en cours..." });
      const result = await generateCreditPDFWithTemplate(credit, {});
      if (result.success) {
        toast({ title: "Téléchargement réussi", description: `L'avoir ${credit.reference} a été téléchargé.` });
      } else { throw new Error(result.error); }
    } catch (error) {
      console.error('Erreur lors du téléchargement:', error);
      toast({ title: "Erreur", description: "Impossible de générer le PDF. Veuillez réessayer.", variant: "destructive" });
    }
  };

  const handlePrint = async (credit: any) => {
    try {
      toast({ title: "Ouverture pour impression", description: `Ouverture de l'avoir ${credit.reference} pour impression...` });
      const result = await printCreditPDFWithTemplate(credit, {});
      if (!result.success) { throw new Error(result.error); }
    } catch (error) {
      console.error('Erreur lors de l\'impression:', error);
      toast({ title: "Erreur", description: "Impossible d'ouvrir le PDF pour impression. Veuillez réessayer.", variant: "destructive" });
    }
  };
  ...
  const formatDate = (dateString: string | null) => {
    ...
    try { return new Date(dateString).toLocaleDateString('fr-FR'); } catch (error) { return '-'; }
  };
      ...
      <>
        {clientCredits.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-muted-foreground mt-1">Ce client n'a pas encore d'avoir.</p>
          </div>
        )}
                  ...
                  <TableRow className="border-t-0">
                    <TableCell colSpan={6} className="py-3 border-t-0">
                      <div className="flex flex-wrap gap-2 justify-end px-4">
                        <Button variant="view" size="sm" onClick={() => handleView(credit)}><Eye className="h-4 w-4 mr-1" />Voir</Button>
                        <Button variant="edit" size="sm" onClick={() => handleEdit(credit)}><Pencil className="h-4 w-4 mr-1" />Modifier</Button>
                        <Button variant="download" size="sm" onClick={() => handleDownload(credit)}><Download className="h-4 w-4 mr-1" />Télécharger</Button>
                        <Button variant="print" size="sm" onClick={() => handlePrint(credit)}><Printer className="h-4 w-4 mr-1" />Imprimer</Button>
                        <Button variant="send" size="sm" onClick={() => handleSendEmail(credit)}><Mail className="h-4 w-4 mr-1" />Envoyer</Button>
                        <Button variant="delete" size="sm" onClick={() => handleDelete(credit)} disabled={deleteCredit.isPending}><Trash className="h-4 w-4 mr-1" />Supprimer</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                </React.Fragment>
