...
const AlternativeCreditPreview = ({ companyData, creditData, clientData, items, totals }: AlternativeCreditPreviewProps) => {
  ...
  const creditDataToUse = creditData || {};
    ...
    <div className="bg-white p-6 rounded shadow-sm h-full" style={{ fontFamily: 'Arial, sans-serif', backgroundColor: 'white', minHeight: '100%' }}>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          <h2 className="text-3xl font-bold text-black">AVOIR N° {creditDataToUse.number || ''}</h2>
          ...
          <div className="text-left p-4 w-2/3 ml-auto">
            <div className="text-sm text-gray-600 space-y-1">
              ...
              {creditDataToUse.licensePlate && <p><strong>Immatriculation :</strong> {creditDataToUse.licensePlate}</p>}
              {creditDataToUse.mileage && <p><strong>Kilométrage :</strong> {creditDataToUse.mileage}</p>}
              {creditDataToUse.vehicle && <p><strong>Véhicule :</strong> {creditDataToUse.vehicle}</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-48 mb-8">
        {creditDataToUse.date && (
          <div className="border-2 border-black rounded-lg px-4 py-2 text-center">
            <div className="font-bold text-sm mb-1">DATE</div>
            <div className="font-bold text-sm">{creditDataToUse.date}</div>
          </div>
        )}
        {creditDataToUse.invoiceNumber && (
          <div className="border-2 border-black rounded-lg px-4 py-2 text-center">
            <div className="font-bold text-sm mb-1">FACTURE N°</div>
            <div className="font-bold text-sm">{creditDataToUse.invoiceNumber}</div>
          </div>
        )}
      </div>
      ...
      {/* Notes */}
      {creditDataToUse.notes && (
        <div className="mt-6">
          <h3 className="text-lg font-bold mb-2">Notes</h3>
          <p className="text-sm">{creditDataToUse.notes}</p>
        </div>
      )}
