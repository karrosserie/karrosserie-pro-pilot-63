    ...
    items: [
      {
        ...
        answer: "L'ajout d'un véhicule nécessite une approche méthodique pour assurer la traçabilité et la conformité : **Prérequis** : Le client propriétaire doit déjà exister dans la base (créez-le d'abord si nécessaire). **Étape 1 - Accès et association** : Dans 'Véhicules', cliquez sur '+ Nouveau véhicule'. Sélectionnez obligatoirement le client propriétaire via le menu déroulant avec recherche automatique (tapez les premières lettres du nom). **Étape 2 - Identification du véhicule** : Plaque d'immatriculation (format français AA-123-AA ou anciens formats acceptés), vérification automatique de validité et détection des doublons. Numéro VIN/Châssis (17 caractères alphanumériques) : saisissez-le pour déclencher le décodage automatique via notre base de données constructeurs mondiale (500+ marques référencées). Le système extrait automatiquement : marque, modèle exact, année de première mise en circulation, cylindrée, puissance fiscale, type de carburant, pays de fabrication. **Étape 3 - Informations techniques manuelles** (si VIN indisponible ou non reconnu) : Marque (liste de 200+ constructeurs avec recherche), Modèle (mise à jour automatique selon la marque), Année de première mise en circulation (1900-année actuelle), Type de véhicule (VP, VUL, moto, etc.), Carburant (essence, diesel, hybride, électrique, GPL, GNV), Cylindrée en cm³, Puissance fiscale en CV, Puissance DIN en kW, Nombre de places assises, Couleur principale et secondaire. **Étape 4 - État et kilométrage** : Kilométrage actuel (avec date de relevé), État général (neuf, très bon, bon, correct, à réviser), Carburant restant (jauge), Pression des pneus, Date du dernier contrôle technique, Date de la dernière révision. **Étape 5 - Documents véhicule** : Carte grise recto/verso (obligatoire), Contrat d'achat ou facture d'acquisition, Attestation d'assurance en cours, Carnet d'entretien/factures de révision, Photos du véhicule (extérieur, intérieur, compteur, défauts visibles). Upload par glisser-déposer, formats PDF/JPEG/PNG, max 10Mo par fichier. **Étape 6 - Informations d'assurance** : Compagnie d'assurance, Numéro de police/contrat, Date d'échéance, Type de garanties (au tiers, intermédiaire, tous risques), Franchise applicable, Coordonnées de l'agent/courtier. **Étape 7 - Validation et traçabilité** : Vérification automatique des champs obligatoires, Contrôle de cohérence (dates, kilométrage croissant), Attribution automatique d'un identifiant unique interne, Création de l'historique vierge pour les futures interventions, Envoi d'un récapitulatif par email au client si configuré. Le véhicule est immédiatement disponible pour la création de devis, ordres de réparation et interventions. Un tableau de bord dédié au véhicule centralise toutes les informations et l'historique complet."
      },
      {
    ...
    items: [
      {
        ...
        answer: "La création d'un ordre de réparation est un processus central qui structure votre intervention de A à Z : **Phase préparatoire** : Assurez-vous que le client et le véhicule existent dans votre base de données. Préparez les informations sur les réparations à effectuer (diagnostic préalable, photos des dommages). **Étape 1 - Initialisation** : Accédez à 'Documents' > 'Ordres de réparation', cliquez sur 'Nouvel ordre'. Sélectionnez le client (recherche par nom/téléphone/email), puis le véhicule parmi ceux du client. Les informations client et véhicule se remplissent automatiquement. **Étape 2 - Informations générales** : Date de réception du véhicule (aujourd'hui par défaut), Date prévisionnelle de début des travaux, Date prévisionnelle de fin (calculée automatiquement selon les temps de main-d'œuvre), Priorité (normale, urgente, très urgente) avec code couleur, Technicien responsable assigné, Poste de travail (carrosserie, peinture, mécanique). **Étape 3 - Diagnostic et réparations** : Description détaillée des défauts constatés, Photos 'avant intervention' (jusqu'à 20 photos), État du véhicule à la réception (propreté, carburant, objets personnels). **Section Réparations** : Pour chaque réparation, cliquez sur '+ Ajouter une réparation' : Description précise du travail (ex: 'Remplacement aile avant droite'), Catégorie (carrosserie, peinture, mécanique, vitrage), Temps de main-d'œuvre estimé (heures et minutes), Taux horaire applicable (peut varier selon le type de travail), Coefficient de difficulté si applicable (majoration pour travaux complexes). **Section Pièces détachées** : Cliquez sur '+ Ajouter une pièce' : Référence constructeur ou équivalent, Description complète, Quantité nécessaire, Prix unitaire HT, Délai d'approvisionnement, Fournisseur habituel. Le système calcule automatiquement les sous-totaux main-d'œuvre et pièces. **Étape 4 - Calculs et conditions** : Application de remises (en % ou montant fixe) par ligne ou globale, Calcul automatique de la TVA (20% standard, autres taux selon réglementation), Total HT, TVA, TTC affiché en temps réel, Conditions de paiement (comptant, 30 jours, échelonné), Modalités particulières (garantie, délai, restrictions). **Étape 5 - Planning et ressources** : Attribution aux techniciens disponibles avec vérification des compétences, Réservation du poste de travail selon la disponibilité, Commande automatique des pièces manquantes, Planification des étapes selon les dépendances (ex: carrosserie avant peinture). **Étape 6 - Validation client** : Génération automatique d'un devis détaillé au format PDF, Envoi par email au client avec demande d'acceptation, Possibilité de signature électronique pour validation, Archivage automatique de l'acceptation client. **Étape 7 - Suivi d'avancement** : L'ordre créé apparaît avec le statut 'En attente' dans le tableau de bord, Modification possible tant que les travaux n'ont pas commencé, Passage au statut 'En cours' dès le début d'intervention, Suivi temps réel de l'avancement avec photos d'étapes, Notification automatique au client des changements de statut. **Fonctionnalités avancées** : Duplication d'ordres similaires pour gain de temps, Templates de réparations courantes, Intégration avec le planning atelier pour optimisation des ressources, Génération de bons de sortie pièces pour le magasin, Traçabilité complète des modifications avec horodatage et utilisateur."
      },
      {
    ...
    items: [
      {
        ...
        answer: "L'enregistrement d'un encaissement suit une procédure rigoureuse pour assurer la traçabilité comptable et la réconciliation bancaire : **Étape 1 - Accès et sélection** : Rendez-vous dans 'Paiements' > 'Encaissements', cliquez sur '+ Nouvel encaissement'. La liste des factures impayées s'affiche automatiquement avec : montant total dû, montant déjà réglé si paiement partiel, solde restant, date d'échéance avec indicateur de retard (rouge si dépassée). **Étape 2 - Sélection de la facture** : Cliquez sur la facture concernée ou utilisez la recherche par numéro/client/montant. Les informations se remplissent automatiquement : numéro de facture, nom du client, montant total, montants déjà encaissés, solde restant dû. **Étape 3 - Détails du paiement** : **Mode de paiement** (obligatoire) : Espèces (avec calcul automatique de rendu de monnaie), Carte bancaire (CB, Visa, Mastercard avec choix du terminal), Virement bancaire (avec référence de virement), Chèque (numéro, banque émettrice, date d'émission), Prélèvement automatique (date de prélèvement), Lettre de change, Compensation (avec une autre créance). **Montant encaissé** : Saisissez le montant réellement perçu (peut être différent du solde pour les paiements partiels), Calcul automatique du solde restant dû, Gestion de l'arrondi pour les paiements espèces, Conversion automatique si devise étrangère. **Étape 4 - Informations complémentaires** : **Référence de paiement** (fortement recommandée) : Numéro de chèque (6-7 chiffres), Numéro de transaction CB (jusqu'à 20 caractères), Référence de virement (communication libre ou structurée), Numéro de traite ou LCR. **Date d'encaissement** (par défaut aujourd'hui, modifiable pour les saisies différées). **Compte bancaire de destination** (sélection parmi vos comptes configurés). **Notes libres** : observations particulières, négociations, conditions spéciales. **Étape 5 - Validation et vérifications** : Contrôle automatique de cohérence (montant positif, inférieur ou égal au solde dû), Vérification de la disponibilité des fonds pour les chèques (si module intégré), Détection des doublons potentiels par montant/date/référence, Aperçu du reçu avant validation finale. **Étape 6 - Génération automatique** : Création d'un reçu de paiement au format PDF avec : En-tête de votre entreprise et coordonnées, Informations du client et de la facture réglée, Détail du paiement avec mode et référence, Calcul du solde restant dû le cas échéant, Mentions légales obligatoires, Signature électronique si configurée. **Étape 7 - Actions post-encaissement** : Envoi automatique du reçu par email au client, Mise à jour du statut de la facture (Payée si solde=0, Paiement partiel sinon), Écriture comptable automatique (débit compte banque, crédit compte client), Mise à jour des tableaux de bord de trésorerie, Notification à l'équipe si montant important. **Gestion des cas particuliers** : **Paiements partiels** : le système garde l'historique de tous les règlements et calcule automatiquement les soldes restants. **Trop-perçu** : possibilité de générer un avoir automatiquement ou de garder un crédit client. **Paiement en plusieurs chèques** : saisie multiple avec détail de chaque chèque et dates d'encaissement différées. **Escompte de règlement** : application automatique de remises pour paiement comptant selon vos conditions commerciales."
      },
      {
