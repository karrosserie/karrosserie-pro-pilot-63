...
const MissionControlDashboard = () => {
  ...
  const getMissionsForPeriod = (period: 'today' | 'week' | 'month', mode: string) => {
    ...
    const allMissions = {
      today: [
        // Ajouter dynamiquement les alertes système filtrées pour aujourd'hui
        ...filteredAlerts
      ...
      week: [
        // Ajouter dynamiquement les alertes système filtrées pour la semaine
        ...filteredAlerts
          .filter(alert => alert.alert_type === 'retard_pointage' || alert.alert_type === 'vehicle_waiting' || alert.alert_type === 'messagerie_urgente')
          .map(alert => {
            if (alert.entity_type === 'employee' && alert.alert_type === 'retard_pointage') {
      ...
      month: [
        // Ajouter dynamiquement les alertes système filtrées pour le mois
        ...filteredAlerts
          .filter(alert => alert.alert_type === 'retard_pointage' || alert.alert_type === 'vehicle_waiting' || alert.alert_type === 'messagerie_urgente')
          .map(alert => {
            if (alert.entity_type === 'employee' && alert.alert_type === 'retard_pointage') {
