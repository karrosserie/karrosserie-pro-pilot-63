        ...
        <Tabs defaultValue="repairs" className="w-full">
            ...
            {repairs.length > 0 && (
              <div className="border-t pt-4 space-y-2">
                <div className="flex justify-end space-x-8 text-sm">
                  <div>Sous-total : <span className="font-medium">{repairTotals.subTotal.toFixed(2).replace('.', ',')} €</span></div>
                  <div>TVA : <span className="font-medium">{repairTotals.totalVat.toFixed(2).replace('.', ',')} €</span></div>
                  <div>Remise TTC : <span className="font-medium">{repairTotals.totalDiscount.toFixed(2).replace('.', ',')} €</span></div>
                </div>
                <div className="flex justify-end text-lg font-bold">
                  Total : <span className="ml-2">{repairTotals.total.toFixed(2).replace('.', ',')} €</span>
                </div>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="parts" className="space-y-4">
            ...
            {parts.length > 0 && (
              <div className="border-t pt-4 space-y-2">
                <div className="flex justify-end space-x-8 text-sm">
                  <div>Sous-total : <span className="font-medium">{partTotals.subTotal.toFixed(2).replace('.', ',')} €</span></div>
                  <div>TVA : <span className="font-medium">{partTotals.totalVat.toFixed(2).replace('.', ',')} €</span></div>
                  <div>Remise TTC : <span className="font-medium">{partTotals.totalDiscount.toFixed(2).replace('.', ',')} €</span></div>
                </div>
                <div className="flex justify-end text-lg font-bold">
                  Total : <span className="ml-2">{partTotals.total.toFixed(2).replace('.', ',')} €</span>
                </div>
              </div>
            )}
          </TabsContent>
