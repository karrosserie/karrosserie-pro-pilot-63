...
import RepairOrderMobileItemCard from './components/RepairOrderMobileItemCard';
...
const AlternativeRepairOrderPreview = ({ companyData, orderData, clientData, vehicleData, items, totals, signatureData }: AlternativeRepairOrderPreviewProps) => {
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded shadow-sm h-full" style={{ fontFamily: 'Arial, sans-serif', backgroundColor: 'white', minHeight: '100%' }}>
      ...
      <div className={`${isMobile ? 'flex flex-col space-y-4' : 'flex justify-between items-start'} mb-6 md:mb-8`}>
        ...
        <div className={isMobile ? '' : 'text-right'}>
          <h2 className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold text-black`}>
            ORDRE DE RÉPARATION N° {orderDataToUse.number || ''}
          </h2>
            ...
            <div className="text-xs md:text-sm text-gray-600 space-y-1">
              ...
              {vehicleData?.start_date && vehicleData?.end_date && (
                <div className="mt-3 pt-2 border-t border-gray-300">
                  <p className="text-base md:text-lg font-semibold text-gray-800 mb-1">Délai prévisionnel</p>
      ...
      {orderDataToUse.date && (
        <div className="flex justify-center mb-6 md:mb-8">
          <div className="border-2 border-black rounded-lg px-3 md:px-4 py-2 text-center">
            <div className="font-bold text-xs md:text-sm mb-1">DATE</div>
            <div className="font-bold text-xs md:text-sm">{orderDataToUse.date}</div>
          </div>
        </div>
      )}

      {/* Tableau des articles - version mobile avec cartes */}
      {itemsToUse.length > 0 && (
        isMobile ? (
      ...
      {(totalsToUse.totalHT || totalsToUse.totalVAT || totalsToUse.totalDiscount || totalsToUse.totalTTC) && (
        <div className={`flex ${isMobile ? 'justify-center' : 'justify-end'}`}>
          {isMobile ? (
            <div className="w-full border-2 border-black rounded-lg p-3 space-y-2 text-sm">
      ...
      {orderDataToUse.notes && (
        <div className="mt-6">
          <h3 className="text-base md:text-lg font-bold mb-2">Notes</h3>
          <p className="text-xs md:text-sm">{orderDataToUse.notes}</p>
        </div>
      )}
      ...
      {signatureData && (
        <div className="mt-6 md:mt-8 flex justify-center">
          <div className="text-center">
            <h3 className="text-base md:text-lg font-bold mb-3 md:mb-4">Signature du client</h3>
            {signatureData.signature ? (
              <div className="border-2 border-black rounded-lg p-3 md:p-4">
                <img 
                  src={signatureData.signature} 
                  alt="Signature du client"
                  className="mx-auto mb-2"
                  style={{ maxWidth: isMobile ? '150px' : '200px', maxHeight: isMobile ? '75px' : '100px' }}
                />
                <p className="text-xs md:text-sm font-bold">{signatureData.clientName}</p>
                {signatureData.signatureDate && (
                  <p className="text-xs text-gray-600">Signé le {signatureData.signatureDate}</p>
                )}
              </div>
            ) : (
              <div className="border-2 border-black border-dashed rounded-lg p-3 md:p-4 text-gray-500">
                <div style={{ width: isMobile ? '150px' : '200px', height: isMobile ? '75px' : '100px' }} className="mx-auto flex items-center justify-center">
                  <span className="text-xs md:text-sm font-bold">Signature en attente</span>
                </div>
              </div>
            )}
          </div>
      ...
      {(companyData?.name || companyData?.address || companyData?.zipcode || companyData?.city || companyData?.siret || companyData?.tva || companyData?.phone || companyData?.email) && (
        <div className="mt-6 md:mt-8 text-xs text-center text-gray-500">
          <p>
            {companyData?.name || ''} 
