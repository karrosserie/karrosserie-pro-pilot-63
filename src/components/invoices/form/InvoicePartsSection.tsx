...
import { InvoicePartItem } from './types';

interface InvoicePartsSectionProps {
  parts: InvoicePartItem[];
  onPartsChange: (parts: InvoicePartItem[]) => void;
  isReadOnly?: boolean;
}

export const InvoicePartsSection = ({ parts, onPartsChange, isReadOnly }: InvoicePartsSectionProps) => {
  const addPart = () => {
  ...
  const updatePart = (id: string, field: keyof InvoicePartItem, value: any) => {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Liste des pièces utilisées pour la réparation
        </CardDescription>
      </CardHeader>
