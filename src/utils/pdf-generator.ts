...
import ReturnAttestationPDF from '@/components/fleet/ReturnAttestationPDF';
import LoanContractPDF from '@/components/fleet/LoanContractPDF';
...
export const generateReturnAttestationPDF = async (returnData: any, loanData: any, companyData: any, userPosition: string) => {
  try {
    // Créer le document PDF
    const doc = ReturnAttestationPDF({ returnData, loanData, companyData, userPosition });
    ...
    // Créer un nom de fichier unique
    const fileName = `attestation-retour-${loanData?.clients?.last_name || 'client'}-${formatDate(new Date().toISOString())}.pdf`;
    ...
    console.error('Erreur lors de la génération du PDF de retour:', error);
    return { success: false, error: error.message };
  }
};

export const generateLoanContractPDF = async (loanData: any, companyData: any, userPosition: string) => {
  try {
    // Créer le document PDF
    const doc = LoanContractPDF({ loanData, companyData, userPosition });
    ...
    // Créer un nom de fichier unique
    const fileName = `contrat-pret-${loanData?.clients?.last_name || 'client'}-${formatDate(new Date().toISOString())}.pdf`;
